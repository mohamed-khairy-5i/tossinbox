...
import { ProviderError, type EmailProvider, type Inbox } from "./types.js";

export interface FailoverOptions {
  /** Provider the user asked for (defaults to the first registered one) */
  requested?: string;
  /** Optional label forwarded to every createInbox attempt */
  label?: string;
  /** When false, a failing provider errors out immediately (no fallback) */
  failover?: boolean;
}

export interface FailoverResult {
  inbox: Inbox;
  /** True when a provider other than the requested one was used */
  switched: boolean;
  /** Short human-readable notes about each failed attempt, in try order */
  warnings: string[];
}
...
function isTransient(err: unknown): boolean {
  if (!(err instanceof ProviderError)) return true; // network-level → transient
  const { status } = err;
...
export async function createInboxWithFailover(
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      if (!isTransient(err) && options.requested) throw err;
