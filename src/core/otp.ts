...
const CODE_KEYWORDS =
  /\b(?:code|otp|passcode|pin|password.?code|verification|verify|confirm|activation|active.?code|one.?time|code.?de.?confirmation)\b/ // en + fr "code de confirmation"
  .source +
  "|" +
  [
    // Arabic
...
export function extractCode(input: string | undefined): string | undefined {
  ...
  for (const line of lines) {
    if (!CODE_KEYWORDS_RE.test(line)) continue;
